...
def refract(vRay, vNormal, fRefractRatio):
    uv = rtu.Vec3.unit_vector(vRay)

    cos_theta = min(rtu.Vec3.dot_product(-uv, vNormal), 1.0)
    sin_theta = math.sqrt(1.0 - cos_theta*cos_theta)

    if fRefractRatio * sin_theta > 1.0:
        return reflect(uv, vNormal)

    perpendicular = (uv + vNormal * cos_theta) * fRefractRatio
    parallel = -vNormal * math.sqrt(abs(1.0 - perpendicular.len_squared()))
...
class Lambertian(Material):
    ...
    def BRDF(self, rView, rLight, hHinfo):
        ...
        return attenuation_color / math.pi
...
class Dielectric(Material):
    ...
    def scattering(self, rRayIn, hHinfo):
        ...
        cos_theta = min(rtu.Vec3.dot_product(-uv, normal), 1.0)
        sin_theta = math.sqrt(1.0 - cos_theta*cos_theta)

        cannot_refract = refract_ratio * sin_theta > 1.0

        # 🔥 เลือก reflect หรือ refract
        if cannot_refract or schlick(cos_theta, self.IOR) > rtu.random_double():
            direction = reflect(uv, normal)
        else:
            direction = refract(uv, normal, refract_ratio)

        origin = hHinfo.getP() + normal * 1e-4
        scattered_ray = rtr.Ray(origin, direction, rRayIn.getTime())

        # 🔥 glass ไม่ลดแสง
        attenuation_color = rtu.Color(1,1,1)
